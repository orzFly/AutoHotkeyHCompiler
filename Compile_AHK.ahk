...
; --------------------------------------------------------------------------------
; Das Script wertet eine im Scriptverzeichnis vorhandene scriptname.ahkh.ini aus
; und steuert damit den Compiler- und ggf. ResHack-Aufruf. Ggf. wird die ini-Datei
; mit Defaultwerten neu angelegt.
; Der vollst鋘dige Pfad des Scripts muss als KommandozeilenParameter 黚ergeben
; werden.
; --------------------------------------------------------------------------------
...
_Select_FILE(ByRef s_FILE , s_TYPE)
{
	...
	Else If s_TYPE = AHK
	{
		...
		s_FILTER := "AutoHotkey Script (*.ahkh)"
		; s_FOLDER := "C:"
	}
	Else If s_TYPE = BIN
	{
...
CommandLine:
	...
	If (IN_EXT != "ahkh") {
		_Error_Exit(Error_Message . "`n""" . In_File . """ is not a .ahkh file.")
	}
